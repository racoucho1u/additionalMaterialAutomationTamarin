...
def parseAndRun(arg):
	...
	try:
		...
		dirname = f"../../files_to_benchmark/noreuselemmas/{caseStudy}"
		...
		if (benchmarkCase == "smartverif"):
			...
			os.system(f"timeout 2h tamarin-prover --prove={lemma} {dirname}/{filename}.spthy {diff} +RTS -N4 -RTS --output={proofFile}  > {bigfile} 2>&1") #--derivcheck-timeout=0
			end = perf_counter()
...
def chooseVersion(version):
	...
	if (version == '0'):
		...
		tamarinDir = "escape/tamarin-prover"
		version = 1
	elif (version == '2'):
		...
		tamarinDir = "escapeBacktrack/tamarin-prover"
		version = 5
	elif (version == '6'):
		benchmarkCase = "escape"
		tamarinDir = "escapeRebase/tamarin-prover"
		version = 6
	elif (version == '7'):
		benchmarkCase = "proba_oldV"
		tamarinDir = "proba_oldV/tamarin-prover"
		version = 7
	else:
		print("Incorrect argument: choose among the following",
				...
				"6: escaping (the real one: rebase)",
				"7: original version of proba",
				sep=os.linesep
			)
		quit()
...
def generateInputFile(filename, diff, lemmaFile, diir, benchmarkcase, constructedInput,log):
	inputs = []
	lemmas = []
	with open(f"../../files_to_benchmark/lemmas/{lemmaFile}") as f:
			lines = f.readlines()
			for line in lines:
				if line[0] != "#":
...
N = 5 
file = "../../files_to_benchmark/input_lemmas_restreint_noreuse"
